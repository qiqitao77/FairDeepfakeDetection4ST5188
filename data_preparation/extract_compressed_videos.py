...
DATASET_PATHS = {
    ...
    'DFD_orignal': 'original_sequences/actors',
    "DFD": 'manipulated_sequences/DeepFakeDetection',
    ...
    if method == 'ffmpeg':
        ...
        while reader.isOpened():
            ...
            if not success:
                break
            if frame_num % 10 == 1:
                cv2.imwrite(join(output_path, '{}.png'.format(frame_num)),
                        image)
            frame_num += 1
        reader.release()
    else:
...
def extract_method_videos(data_path, dataset, compression):
    ...
    images_path = join(data_path, DATASET_PATHS[dataset], compression, 'face_images')
