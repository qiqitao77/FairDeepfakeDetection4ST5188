...
def create_augmented_images(image_path, num_augmented_images=5, save_dir='./augmented_images'):
    ...
    for idx, aug_image in enumerate(augmented_images, start=0):
        save_image(aug_image, save_dir, base_name, idx)
...
for key in data.keys():
    ...
    if origin_data_num[idx] <= data_num[idx]:
        ...
        if init_num[idx] < need_num:
            create_augmented_images(item['image_path'], count_num, save_dir='./augmented_images')
            ...
            create_augmented_images(item['image_path'], count_num - 1, save_dir='./augmented_images')
